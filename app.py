...
app = Flask(__name__)
CORS(app)  # Blogger se cross-origin request allow karne ke liye
...
# Auto-detected box coordinates (x0, y0, x1, y1) — template image ke andar
PHOTO_BOX      = (141, 274, 483, 709)     # left bada box — Farmer photo
QR_BOX         = (1166, 459, 1347, 717)   # right chhota box — QR code
FARMER_ID_BOX  = (518, 747, 1038, 897)    # neeche golden box — Farmer ID
...
# Fonts — inhe GitHub repo mein "fonts/" folder ke andar rakhna
# (Google Fonts se "Poppins" download kar lena — free hai)
...
def get_font(bold, size):
    ...
    try:
        ...
        # Agar font file na mile to fallback (basic look, lekin crash nahi hoga)
...
def extract_farmer_data(pdf_bytes):
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        ...
        # Photo: PDF mein embedded sabse badi image nikalo
        photo_img = None
        if page.images:
            biggest = max(
                page.images,
...
def cover_fit(img, box_w, box_h):
    """Photo ko box ke andar bina stretch/distort kiye 'cover' karke fit karta hai."""
...
def draw_label_value(draw, box, label, value, label_size=28, value_gap=10):
    """Box ke andar 'Bold Label : Normal Value' style mein likhta hai, vertically center."""
    x0, y0, x1, y1 = box
    box_h = y1 - y0
...
def generate_card():
    ...
    # Agar template ka size reference size se alag hai to boxes proportionally scale karo
    ...
    # ---- Name (bada font, bold nahi) ----
    name_font_size = int(48 * scale_y)
    draw_centered_text(draw, name_box, data["name"], size=name_font_size, bold=False)

    # ---- DOB / Gender / Caste / Mobile (Bold Label : Normal Value) ----
    label_size = int(26 * scale_y)
    ...
    id_font_size = int(34 * scale_y)
    draw_centered_text(draw, farmer_id_box, f"Farmer ID : {farmer_id}", size=id_font_size, bold=True)

    # ---- QR Code ----
    qr_size = min(qr_box[2] - qr_box[0], qr_box[3] - qr_box[1])
    qr_img = make_qr(AGRISTACK_URL, qr_size)
    qx = qr_box[0] + ((qr_box[2] - qr_box[0]) - qr_size) // 2
    qy = qr_box[1] + ((qr_box[3] - qr_box[1]) - qr_size) // 2
